...
typedef enum TokenType { FloatDeclaration, IntegerDeclaration,
                         PrintOp, AssignmentOp,
...
typedef struct AssignmentStatement {
    ...
    Expression *expr;
    DataType   type; 
        /* For type checking to store the type of all expression on the right. */
} AssignmentStatement;
